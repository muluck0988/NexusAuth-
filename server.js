...
// Banco de dados simples para Tokens (em produção, use MongoDB/PostgreSQL)
const DB_PATH = path.join(__dirname, 'tokens.json');
if (!fs.existsSync(DB_PATH)) fs.writeFileSync(DB_PATH, JSON.stringify({}));

function getTokens() { return JSON.parse(fs.readFileSync(DB_PATH)); }
function saveTokens(tokens) { fs.writeFileSync(DB_PATH, JSON.stringify(tokens, null, 2)); }
...
app.post('/api/admin/generate-token', (req, res) => {
  ...
  if (adminKey !== 'NEXUS_ADMIN_2026') return res.status(403).json({ success: false, error: 'Acesso negado' });
...
app.post('/api/auth/validate', (req, res) => {
  ...
  if (!tokens[token]) return res.status(401).json({ success: false, error: 'Token inválido' });
...
app.post('/api/auth/reset-ip', (req, res) => {
  ...
  const tData = tokens[token];
  if (tData.password !== password) return res.status(403).json({ success: false, error: 'Senha incorreta' });
 ...
 */
apapp.post('/api/tempmail/create', protectApi, (req, res) => {=> {
  try {
